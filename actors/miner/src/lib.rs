        ...
        RT: Runtime<BS>,
    {
        ...
        rt.transaction(|state: &mut State, rt|{
            ...
            for (i, precommit) in params.sectors.iter().enumerate() {
                ...
                fvm_sdk::debug::log(format!("fvm this_epoch_reward_smoothed: {:?}", &reward_stats.this_epoch_reward_smoothed));
                fvm_sdk::debug::log(format!("fvm quality_adj_power_smoothed: {:?}", &power_total.quality_adj_power_smoothed));
